...
bool applyStrengthReduction(ExpressionTree &tree)
{
    ...
    tree.getRoot()->postorder([&](ExpressionTreeNode &node)
    {
        ...
        //
        // Only expand small exponents: a large exponent (from an untrusted expression string) would build a
        // huge tree, blowing up memory/CPU in the optimizer (effective DoS), and casting an out-of-int-range
        // float exponent is undefined behavior. Larger exponents are left as a regular POW op.
        constexpr int maxIntegerPowExpansion = 256;
        if (node.op == ExprOpType::POW && isConstant(*node.right) && isInteger(node.right->op.imm.f) && node.right->op.imm.f > 0 && node.right->op.imm.f <= maxIntegerPowExpansion) {
